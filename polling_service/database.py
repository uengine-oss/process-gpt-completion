...
def fetch_and_delete_stale_new_workitems() -> int:
    """
    조건에 맞는 워크아이템을 조회하고 삭제합니다.
    
    조건:
    - proc_def_id와 proc_inst_id가 null이 아님
    - activity_id가 null이 아님
    - updated_at이 현재 시간에서 30분 이상 이전
    - proc_inst_id 값으로 bpm_proc_inst 테이블을 조회했을 때 값이 존재하는 row
    ...
    try:
        ...
        while True:
            # 조건에 맞는 워크아이템을 최대 100개까지 조회
            # proc_def_id, proc_inst_id, activity_id가 null이 아니고, updated_at이 30분 이전인 항목들
            response = (
                supabase.table('todolist')
                .select('id, proc_inst_id, tenant_id, updated_at')
                .not_.is_('proc_def_id', 'null')
                .not_.is_('proc_inst_id', 'null')
                .not_.is_('activity_id', 'null')
                .eq('tenant_id', 'uengine')
                .eq('status', 'IN_PROGRESS')
                .lt('updated_at', thirty_minutes_ago)
                .limit(100)
                .execute()
            )
            
            batch = response.data or []
            print(f"[DEBUG] fetch_and_delete_stale_new_workitems batch size: {len(batch)}")
            ...
            # 각 워크아이템에 대해 bpm_proc_inst 테이블에 존재하지 않는 orphan 인지 확인 후 삭제
            for workitem in batch:
                proc_inst_id = workitem.get('proc_inst_id')
                tenant_id = workitem.get('tenant_id')
                workitem_id = workitem.get('id')
                updated_at = workitem.get('updated_at')
                
                if not proc_inst_id or not tenant_id:
                    continue
                ...
                try:
                    ...
                    # bpm_proc_inst 에 존재하지 않는 경우에만 삭제
                    if not inst_response.data or len(inst_response.data) == 0:
                        try:
                            supabase.table('todolist').delete().eq('id', workitem_id).eq('tenant_id', tenant_id).execute()
                            batch_deleted_count += 1
                            print(f"[INFO] Deleted orphan workitem {workitem_id} (proc_inst_id: {proc_inst_id}, updated_at: {updated_at})")
                        except Exception as delete_error:
                            print(f"[ERROR] Failed to delete workitem {workitem_id}: {str(delete_error)}")
                            
                except Exception as check_error:
                    print(f"[ERROR] Failed to check bpm_proc_inst for workitem {workitem_id}: {str(check_error)}")
                    continue

            total_deleted_count += batch_deleted_count

            # 이번 배치에서 100개 미만을 처리했다면 더 이상 남은 후보가 없다고 보고 종료
            if len(batch) < 100:
                break

        if total_deleted_count > 0:
            print(f"[INFO] Deleted {total_deleted_count} stale orphan workitems")
        
        return total_deleted_count
        
    except Exception as e:
        print(f"[ERROR] Failed to fetch and delete stale orphan workitems: {str(e)}")
        return 0
