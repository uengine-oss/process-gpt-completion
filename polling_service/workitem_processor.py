...
async def check_event_expression(next_activity_payloads: list[dict], chain_input_next: dict) -> list[dict]:
    ...
    try:
        ...
        chain_input_text = {
            "instruction": (
                "당신은 BPMN 타이머 이벤트 플래너입니다. runtimeContext와 각 nlText를 읽고 "
                "타이머를 엄격하게 계산하세요.\n\n"
                "[핵심 목표]\n"
                "- 사용자가 의도한 시점에 정확히 1회 또는 의도된 반복으로만 실행되게 설정\n"
                "- 근거 없는 반복 규칙화(예: 단발 일정을 매년 반복으로 일반화) 절대 금지\n\n"
                "[입력 사용 제약]\n"
                "- runtimeContext만 사용\n"
                "- 해석 불가/근거 부족 시 expression, dueDate 모두 빈 문자열\n\n"
                "[출력 규칙]\n"
                "- 반드시 JSON: {\"timers\": [{\"id\": \"...\", \"expression\": \"\", \"dueDate\": \"\"}]}\n"
                "- 각 후보 id마다 1개 결과\n"
                "- expression 또는 dueDate 중 하나만 채움(둘 다 채우지 않음)\n"
                "- dueDate는 시간 정보가 필요 없을 때만 YYYY-MM-DD 사용\n\n"
                "[cron 필드 규칙(중요)]\n"
                "- 5필드: \"m h dom mon dow\" (pg_cron 직접 형식)\n"
                "- 6필드: \"sec m h dom mon dow\" (Quartz without year)\n"
                "- 7필드: \"sec m h dom mon dow year\" (Quartz with year)\n"
                "- 단발성(절대 날짜/시간이 특정됨) 일정은 반드시 7필드로 생성하여 year를 포함\n"
                "- 반복 일정(매일/매주/매월 등)만 5필드 또는 6필드 사용\n"
                "- dom 또는 dow에 '?'가 필요한 Quartz 문맥에서는 사용 가능\n\n"
                "[단발성 판정 규칙]\n"
                "- runtimeContext에서 연/월/일/시/분이 특정되면 단발성으로 간주\n"
                "- 예: \"2026-03-16 13:00\", \"2026년 3월 16일 오후 1시\" → 단발성\n"
                "- \"강의 시작 30분전\"처럼 상대시간이면 기준 시각(runtimeContext의 실제 강의 시작 시각)에서 정확히 계산\n"
                "- 계산 결과가 단발성이면 7필드 cron으로 반환\n\n"
                "[금지 사항]\n"
                "- 단발성 근거가 있는데 5필드/6필드로 축약 금지\n"
                "- 날짜 정보 일부가 부족한 상태에서 임의로 year/month/day를 추정 금지\n"
                "- \"매년\", \"매월\", \"매주\", \"매일\" 표현이 없는데 반복으로 만들지 말 것\n\n"
                "[예시]\n"
                "- 입력 의미: \"강의 시작 30분 전 알림\", 시작시각=2026-03-16 13:00\n"
                "  -> expression: \"0 30 12 16 3 ? 2026\" (정답)\n"
                "  -> expression: \"30 12 16 3 *\" (오답: 매년 반복으로 해석될 수 있음)\n"
            ),
